...
- Every `@quantajs/vue` composable: `useQuantaValue` (with `shallow` for a projection, and an action's `pending` state), `useQuanta`, `useQuantaActions`, and `useLocalStore` in two components that each get their own store.
